...
public class FavoriteTeamService {
    ...
    public FavoriteTeamResponse setFavoriteTeam(Long userId, FavoriteTeamRequest req) {
        ...
        if (team == null) {
            return new FavoriteTeamResponse(false, "존재하지 않는 팀 ID입니다.");
        }
        ...
        return favoriteTeamRepository.findByUserId(userId)
                .map(ft -> {
                    ...
                    return new FavoriteTeamResponse(true, "선호팀이 정상적으로 변경되었습니다.");
                })
                .orElseGet(() -> {
                    ...
                    return new FavoriteTeamResponse(true, "선호팀이 정상적으로 등록되었습니다.");
                });
    }
    ...
    public FavoriteTeamResponse updateFavoriteTeams(Long userId, List<FavoriteTeamRequest> reqs) {
        ...
        if (reqs == null || reqs.isEmpty()) {
            return new FavoriteTeamResponse(false, "최소 한 개의 선호팀은 선택해야 합니다.");
        }

        // 우선순위 중복 체크
        Set<Integer> orders = new HashSet<>();
        for (FavoriteTeamRequest r : reqs) {
            if (!orders.add(r.getDisplayOrder())) {
                return new FavoriteTeamResponse(false, "우선순위가 중복되었습니다: " + r.getDisplayOrder());
            }
            // 팀 ID 유효성 검증
            if (Team.fromId(r.getTeamId()) == null) {
                return new FavoriteTeamResponse(false, "존재하지 않는 팀 ID입니다: " + r.getTeamId());
            }
        }
        ...
        return new FavoriteTeamResponse(true, "선호팀 목록이 정상적으로 저장되었습니다.");
    }
}
